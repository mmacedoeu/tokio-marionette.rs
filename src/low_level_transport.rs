...
/// reactor.
pub struct LowLevelMarionetteTransport<T> {
    inner: T,
    read_buffer: Vec<u8>,
    write_buffer: io::Cursor<Vec<u8>>,
}

pub fn new_marionette_transport<T>(inner: T) -> LowLevelMarionetteTransport<T>
    where T: Io
{
    LowLevelMarionetteTransport {
        inner: inner,
        read_buffer: vec![],
...
impl<T> FramedIo for LowLevelMarionetteTransport<T>
    where T: Io
{
    type In = Frame;
    type Out = Frame;
    ...
    fn read(&mut self) -> Poll<Frame, io::Error> {
        loop {
            ...
            if let Some(n) = self.read_buffer.iter().position(|b| *b == b':') {
                debug!("{}", str::from_utf8(&self.read_buffer).unwrap());
                let bytevar = self.read_buffer[0..n].to_vec();
                let bytestring = String::from_utf8(bytevar).unwrap();
                let bytenumber = bytestring.parse::<usize>().unwrap();
                if self.read_buffer.len() - n >= bytenumber {
                    let tail = self.read_buffer.split_off(bytenumber + n + 1);
                    let line = mem::replace(&mut self.read_buffer, tail);
                    return String::from_utf8(line[n+1..line.len()].to_vec())
