...
/// requirements.
struct MarionetteService<T> {
    inner: T,
}

impl<T> Service for MarionetteService<T>
...
pub fn serve<T>(handle: &Handle, addr: SocketAddr, new_service: T) -> io::Result<()>
    where T: NewService<Request = String, Response = String, Error = io::Error> + Send + 'static
{
    try!(server::listen(handle, addr, move |stream| {
        ...
        // transport
        let service = MarionetteService { inner: try!(new_service.new_service()) };
